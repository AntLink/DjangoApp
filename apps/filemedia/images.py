...
# thumb = [160, 320, 384, 280, 373, 507, 538, 498, 480, 640, 800, 960, 1120, 1280, 1440, 1600]
...
# thumb = [160, 320, 384, 280, 373, 507, 533, 538, 498, 480, 640, 800, 960, 1120, 1280, 1440, 1600, 1920]
thumb = [1920]


class ManageImage(object):

    def crop_image(self, src, dst, width, height):
        try:
            with PilImage.open(src) as img:
                # Pastikan gambar cukup besar untuk di-crop
                if img.size[0] < width or img.size[1] < height:
                    return False

                # Hitung titik awal crop (tengah gambar)
                ...
                # Perbaiki kualitas (harus 1-95 untuk JPEG)
                quality = min(95, 120)
                if newimg.mode == 'RGBA':
                    newimg = newimg.convert('RGB')
                return newimg.save(dst, quality=quality, optimize=True)
    ...
    def resize_image_thumb(self, src, dst, width):
        try:
            ...
            print(f"Error resizing thumb: {e}")
            return False
    ...
    def resize_image(self, src, dst, width):
        try:
            with PilImage.open(src) as img:
                ...
                quality = min(95, 120)
                return img.save(dst, quality=quality, optimize=True)
        except (IOError, OSError) as e:
    ...
    def delete_dir_if_empty(self, dir_path):
        try:
    ...
    def delete_all_image(self, path, name):
        # Hapus gambar utama
        self.delete_img(os.path.join(path, name))

        thumbnail_path = os.path.join(path, "thumbnail")
        list_dir =  os.listdir(thumbnail_path)
        # Hapus thumbnail dari semua ukuran
        for size in list_dir:
            thumb_path = os.path.join(path, 'thumbnail', str(size), name)
            self.delete_img(thumb_path)

            # Hapus folder jika kosong
            size_dir = os.path.dirname(thumb_path)
            self.delete_dir_if_empty(size_dir)

        # Hapus folder thumbnail utama jika kosong
        thumb_dir = os.path.join(path, 'thumbnail')
        self.delete_dir_if_empty(thumb_dir)

        # Hapus folder utama jika kosong
        self.delete_dir_if_empty(path)

    def compress_image(self, img_dir, file_name):
        thumb_dir = os.path.join(img_dir, 'thumbnail')
        self.create_directory(thumb_dir)
        ...
        for size in thumb:
            ...
            dst = os.path.join(size_dir, file_name)

            # Resize gambar
            self.resize_image_thumb(src, dst, size)

            # Crop untuk ukuran tertentu (jika diperlukan)
            if size in [160, 320, 384, 280, 373, 507, 533, 538, 498, 480, 640, 800, 960, 1120, 1280, 1440, 1600, 1920]:  # Sesuaikan dengan kebutuhan crop
                self.crop_image(dst, dst, size, size)
