...
$(document).on('nifty.ready', function () {
    $('.dropzone').on('click', function () {
        ...
        $("#img-" + name).dropzone({
            ...
            thumbnailHeight: 150,
            maxFiles: 1,
            previewTemplate: '<div class="dz-preview"><img src="/static/admin/filemedia/img/no-thumb/td_100x100.png" class="img-responsive img-rounded dz-clickable" style="width: 160px"/><div class="dz-progress" style="width: 110px;margin-left: 25px; margin-top: -80px; position: absolute;"><div class="progress progress-striped active"><div data-dz-uploadprogress class="progress-bar progress-bar-success"></div></div></div></div>',
            success: function (file, response) {
                if (response.status) {
                    ...
                    $(file.previewTemplate).find('img').attr('data-id', response.id);
