...
class Category(models.Model):
    ...
    def get_assets_count(self):
        # Hitung jumlah media di kategori ini
        return Media.objects.filter(category=self).count()

    def get_total_assets_count(self):
        # Hitung jumlah total media termasuk sub-kategori
        count = self.get_assets_count()
        for child in self.folders.all():
            count += child.get_total_assets_count()
        return count
...
class Folder(models.Model):
    ...
    class Meta:
        ...
        verbose_name_plural = _('folderss')
...
class Media(models.Model):
    ...
    tags = models.ManyToManyField(
        Tags,
        through='Mediahastags',  # 👉 pakai tabel relasi buatan
        blank=True,
        verbose_name=_('Tags'),
        related_name="media_tags"
    )
    category = models.ForeignKey(Category, on_delete=models.CASCADE, related_name="media_category", null=True, blank=True)
    folder = models.ForeignKey(Folder, on_delete=models.CASCADE, null=True, blank=True, related_name="media_folder")
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['category', 'folder', 'type', 'favored']),
        ]
...
class Image(Media):
    ...
    def get_image(self, size=None):
        if size == 'no':
            ...
            file = settings.BASE_DIR + os.sep + 'media' + os.sep + 'images' + os.sep + self.path + os.sep + 'thumbnail' + os.sep + str(size) + os.sep + self.unique_name
            if os.path.exists(file):
                return mark_safe(image)
            else:
                return mark_safe(no_thumb)
    ...
    def tags(self):
        html = "&nbsp;".join([u'<a href="%s" class="btn-link"><span class="label label-info">%s</span></a>' % (reverse('admin:%s_%s_tags' % (self._meta.app_label, self._meta.model_name), args=[p.pk, 'list']), p.name) for p in self.tags.all()])
...
class Mediahastags(models.Model):
    ...
    class Meta:
        db_table = "ant_media_tag_relations"  # 👉 beri nama tabel unik agar jelas
