RedactorX.add('plugin', 'emoji', {
    ...
    start: function() {
        var t = {
            title: "## emoji.emoji ##",
            icon: this.opts.emoji.icon,
            command: "emoji.popup",
            blocks: {all: "editable"}
        };

        this.app.toolbar.add("emoji", t);

        if (this.opts.emoji.context) {
            this.app.context.add("emoji", t);
        }
    },
    popup: function(t, e) {
        var items = this.buildItems();

        this.app.popup.create("emoji", {
            type: "grid",
            width: "450px",
            items: items
        });
    ...
    insert: function(t) {
        ...
        this.app.insertion.insertHtml(t.emoji, "after");
    },
    buildItems: function() {
        var items = {};
        var emojiItems = this.opts.emoji.items;
        var index = 0;

        // Membuat item-item emoji secara flat
        Object.entries(emojiItems).forEach(([category, emojis]) => {
            Object.entries(emojis).forEach(([name, emoji]) => {
                items[index] = {
                    // Hapus properti title untuk menghindari duplikasi
                    // title: emoji,
                    name: name,
                    html: this.dom('<div>').addClass(this.prefix + '-popup-item').html(emoji),
