...
class MediahastagsInline(admin.TabularInline):
    model = Mediahastags
    extra = 1

class ImageAdmin(Admin):
    inlines = [MediahastagsInline]
    user_req = None
    form = ImageForm
    ...
    def ajax_image_upload_view(self, request):
        ...
        if request.POST.get('object_id') != 'None' and request.POST.get('object_id') != None and request.POST.get('object_id') != '':
            ...
            if (request.POST.get('taxid')):
                img.tags.add(int(request.POST.get('taxid')))
        ...
        mi.compress_image(img_dir, file_name)
    ...
    def get_queryset(self, request):
        if self.tax:
            if request.user.is_superuser:
                return self.model.objects.filter(type='p', tags=self.tax)
            else:
                return self.model.objects.filter(type='p', tags=self.tax, user=request.user)
        else:
            if request.user.is_superuser:
...
class FileAdmin(Admin):
    ...
    fieldsets = [
        (None, {'fields': ('name', 'files', 'description')}),
    ]
    # list_filter = ['created_at']
    ...
    def ajax_file_upload_view(self, request):
        ...
        if request.POST.get('object_id') != 'None' and request.POST.get('object_id') != None and request.POST.get('object_id') != '':
            ...
            if (request.POST.get('taxid')):
                file.tags.add(int(request.POST.get('taxid')))
    ...
    def get_queryset(self, request):
        if self.tax:
            if request.user.is_superuser:
                return self.model.objects.filter(type='f', tags=self.tax)
            else:
                return self.model.objects.filter(type='f', tags=self.tax, user=request.user)
        else:
            if request.user.is_superuser:
