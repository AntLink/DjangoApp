...
@simple_tag(takes_context=True)
def get_menu_user(context, request):
    """
    :type request: HttpRequest
    """
    if not isinstance(request, HttpRequest):
    ...
    return Menu(context, request, app_list, 'users').get_app_list()


@simple_tag(takes_context=True)
def get_menu_header(context, request):
    """
    :type request: HttpRequest
    """
    if not isinstance(request, HttpRequest):
    ...
    return Menu(context, request, app_list, 'header').get_app_list()


@simple_tag(takes_context=True)
def get_menu_user_header(context, request):
    """
    :type request: HttpRequest
    """
    if not isinstance(request, HttpRequest):
    ...
    return Menu(context, request, app_list, 'user_header').get_app_list()


@simple_tag(takes_context=True)
def get_menu_shortcut(context, request):
    """
    :type request: HttpRequest
    """
    if not isinstance(request, HttpRequest):
    ...
    return Menu(context, request, app_list, 'shortcut').get_app_list()
...
class Menu(object):
    ...
    def __init__(self, context, request, app_list, menu_type='default'):
    ...
    def init_config(self):
        ...
        if self.menu_type == 'users':
            self.conf_exclude = get_config('MENU_USERS_EXCLUDE')
            self.conf_open_first_child = get_config('MENU_USERS_OPEN_FIRST_CHILD')
            self.conf_icons = get_config('MENU_USERS_ICONS')
            self.conf_menu_order = get_config('MENU_USERS_ORDER')
            self.conf_menu = get_config('MENU_USERS')

        if self.menu_type == 'header':
            self.conf_exclude = get_config('MENU_HEADER_EXCLUDE')
            self.conf_open_first_child = get_config('MENU_HEADER_OPEN_FIRST_CHILD')
            self.conf_icons = get_config('MENU_HEADER_ICONS')
            self.conf_menu_order = get_config('MENU_HEADER_ORDER')
            self.conf_menu = get_config('MENU_HEADER')

        if self.menu_type == 'user_header':
            self.conf_exclude = get_config('MENU_USER_HEADER_EXCLUDE')
            self.conf_open_first_child = get_config('MENU_USER_HEADER_OPEN_FIRST_CHILD')
            self.conf_icons = get_config('MENU_USER_HEADER_ICONS')
            self.conf_menu_order = get_config('MENU_USER_HEADER_ORDER')
            self.conf_menu = get_config('MENU_USER_HEADER')

        if self.menu_type == 'shortcut':
            self.conf_exclude = get_config('MENU_SHORCUT_EXCLUDE')
            self.conf_open_first_child = get_config('MENU_SHORCUT_OPEN_FIRST_CHILD')
            self.conf_icons = get_config('MENU_SHORCUT_ICONS')
            self.conf_menu_order = get_config('MENU_SHORCUT_ORDER')
            self.conf_menu = get_config('MENU_SHORCUT')
